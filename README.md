...
Network Mode exposes person terms in Create and is configured through `Company Rules -> People Rules`. People Rules stores shared person rows in `ASSIGNMENT RULES\seller_terms.csv`. The UI labels `Seller Rate %`, `Deduction %`, and `Balance Share %` as percentage fields and accepts numbers only, for example `90`, `92.5`, `10`, or `10.5`, without percent signs. Optional `Min Value` / `Max Value` fields are plain dollar-value cutoffs; blank bounds default to `$0` and `$1,000,000,000`. `Seller Rate %` pays the seller a flat percentage of the selected Sheet Type company's source value. `Deduction %` follows that Sheet Type company's payout logic and subtracts the deduction from the company payout. A blank `Sheet Type` with `Balance Share %` defines a team member profit share, such as `50`, without making that person a seller source.
