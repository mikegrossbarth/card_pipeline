...
- Summary: Home has a sheet search field under the sort control. Team Profit exposes `My Profit` as a special Person dropdown selection; `My Profit` subtracts team balance-share payouts from per-card team-member profit rows while leaving seller/network payout rows already net. When People Rules has explicit `Balance Share %` rows, the Team Profit Person selector only offers those balance-share team members plus `My Profit`; seller/network people are intentionally hidden because their purchase basis is seller-term sheet economics. Payouts/Tabs has a `Total` / `Per Sheet` summary view, where `Per Sheet` is seller/network-sheet oriented and not for balance-share team employees. Payout balance rows can now record partial payments through `paid_amount`, show remaining open balance as `Partial`, and still preserve paid-in-full behavior. Startup Home loading uses cached workbook summaries immediately and refreshes changed/missing summaries in the background so cold Google Drive workbook scans no longer block the first usable UI.
- Notes / avoid porting: Do not reintroduce a separate Profit `View` dropdown for `My Profit`; keep it as a Person dropdown option. Do not revert team-member payout math to sheet-level grouping. Team members stay per profit-ledger item; seller/network payouts can be sheet based. Do not model partial payments as fake profit rows.
