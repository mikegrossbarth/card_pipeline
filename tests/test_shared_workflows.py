...
class SharedStateTests(unittest.TestCase):
    ...
    def test_scan_to_cert_preserves_old_sgc_dashed_cert_numbers(self) -> None:
        ...
        self.assertEqual(bridge_normalize_result_cert("SGC 2007430-003"), "2007430-003")
        self.assertEqual(bridge_cert_match_key("2007430-003"), bridge_cert_match_key("2007430003"))
...
class DiagnosticsTests(unittest.TestCase):
    def test_setup_doctor_reports_pipeline_and_google_paths(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("LUCAS Windows", lucas_diagnostics.lucas_version_label("Windows"))
...
class AssignmentEngineTests(unittest.TestCase):
    ...
    def test_assignment_decision_reports_selected_payout_tier(self) -> None:
        ...
        engine = assignment_engine.AssignmentEngine(
            [
                assignment_engine.AssignmentCompany(
                    ...
                    [
                        assignment_engine.PayoutTier(0, 1000, 0.9, "Baseball"),
                        assignment_engine.PayoutTier(100, 1000, 0.95, "Vintage"),
                    ],
                )
            ]
        )
        ...
        self.assertEqual(decision.payout_category, "Vintage")
...
class AppSharedWorkflowLogicTests(unittest.TestCase):
    ...
    def test_save_comp_to_source_sheet_recalculates_seller_purchase(self) -> None:
        ...
        class SourceSaveDummy:
            ...
            _seller_terms_match_for_row = lambda self, seller, sheet_type, row: ({"seller": seller, "sheet_type": sheet_type, "rate": 0.9}, None)
            _seller_term_for_marker = app.CardPipelineApp._seller_term_for_marker
            _seller_terms_company_decision = app.CardPipelineApp._seller_terms_company_decision
            _seller_terms_company_price = app.CardPipelineApp._seller_terms_company_price
            _seller_terms_value_in_range = app.CardPipelineApp._seller_terms_value_in_range
    ...
    def test_people_rules_sheet_type_disables_balance_share_entry(self) -> None:
        dialog = assignment_config_ui.PeopleRulesDialog.__new__(assignment_config_ui.PeopleRulesDialog)
    ...
    def test_right_click_move_to_received_syncs_sheet_inventory(self) -> None:
        ...
        class MoveDummy:
            ...
            def __init__(self, root: Path):
                ...
                self.home_sheet_markers = {"Incoming|Lot A.xlsx": {"assigned_person": "Mikey"}}
        ...
        with TemporaryDirectory() as tmp:
            ...
            try:
                ...
                with patch.object(app.messagebox, "askyesno", return_value=True), patch.object(app, "shared_lock", return_value=NullLock()):
    ...
    def test_record_profit_sales_updates_stale_inventory_sale_duplicate(self) -> None:
        class ProfitDummy:
            _load_profit_ledger = app.CardPipelineApp._load_profit_ledger
            _save_profit_ledger = app.CardPipelineApp._save_profit_ledger
            _profit_record_key = app.CardPipelineApp._profit_record_key
        ...
        with TemporaryDirectory() as tmp:
            ...
            dummy = ProfitDummy()
            dummy.lucas_identity = {"display_name": "Tester", "machine": "Test"}
            stale = dummy._normalize_profit_record(
                {
                    "assigned_person": "Kevin Hambone",
                    "cert_number": "49960916",
                    "grader": "PSA",
                    "card_title": "1988 Fleer Michael Jordan PSA 9",
                    "company": "General Sold",
            ...
            try:
                dummy._save_profit_ledger([stale])
                self.assertEqual(
                ...
                self.assertEqual(len(ledger), 1)
                self.assertEqual(ledger[0]["sale_price"], 1800.0)
    ...
    def test_inventory_filter_finds_missing_comps(self) -> None:
        class FieldVar:
            def __init__(self, value=""):
                self.value = value
        ...
        dummy.inventory_missing_photos_var = FieldVar(False)
        rows = [
            {"status": "Active", "cert_number": "111", "card_title": "Needs Comps", "card_ladder_comps_average": None},
            {"status": "Active", "cert_number": "222", "card_title": "Also Needs Comps", "card_ladder_comps_average": ""},
            {"status": "Active", "cert_number": "333", "card_title": "Has Comps", "card_ladder_comps_average": 125.0},
            {"status": "Sold", "cert_number": "444", "card_title": "Sold Missing Comps", "card_ladder_comps_average": None},
        ]

        self.assertEqual([row["cert_number"] for row in dummy._filtered_inventory_records(rows)], ["111", "222"])

    def test_inventory_filter_finds_missing_cl_values(self) -> None:
        class FieldVar:
            def __init__(self, value=""):
                self.value = value
        ...
        dummy.inventory_date_max_var = FieldVar("")
        dummy.inventory_missing_title_var = FieldVar(False)
        dummy.inventory_missing_comps_var = FieldVar(False)
    ...
    def test_inventory_sport_filter_accepts_multiple_checked_sports(self) -> None:
        ...
        rows = [
            {"status": "Active", "sport": "football", "cert_number": "1"},
            {"status": "Active", "sport": "basketball", "cert_number": "2"},
            {"status": "Active", "sport": "baseball", "cert_number": "3"},
            {"status": "Active", "sport": "pokemon", "cert_number": "4"},
        ]
    ...
    def test_home_all_received_marker_adds_received_sheet_inventory(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertTrue(str(inventory[2]["item_id"]).startswith(f"RAW-TEAM-{datetime.now().strftime('%Y%m%d')}-"))
                self.assertEqual(inventory[2]["card_title"], "Raw Card Without Sheet ID")
                self.assertEqual(inventory[3]["item_type"], "Raw")
                self.assertTrue(str(inventory[3]["item_id"]).startswith(f"RAW-TEAM-{datetime.now().strftime('%Y%m%d')}-"))
