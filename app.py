...
class CardPipelineApp(tk.Tk):
    ...
    def open_inventory_recomp_popup(self) -> None:
        ...
        ttk.Label(frame, text="Comp Method", style="Muted.TLabel").grid(row=4, column=0, sticky="w", pady=(0, 6))
        ttk.Combobox(frame, textvariable=strategy_var, values=list(COMP_STRATEGY_DISPLAY.keys()), width=24, state="readonly").grid(row=4, column=1, sticky="w", pady=(0, 6))
        ttk.Label(frame, text="Windows reads CY from sheets but does not run CY automation.", style="Muted.TLabel").grid(row=5, column=0, columnspan=2, sticky="w", pady=(4, 12))
    ...
    def recomp_inventory_visible_rows(self, features: dict[str, object] | None = None) -> None:
        features = dict(features or {"card_ladder_value": True, "card_ladder_comps": True, "cy": False, "strategy_label": self.comp_strategy_label.get()})
        ...
        if not temp_rows:
            ...
            self.inventory_status_var.set("No visible inventory rows are eligible for Card Ladder recomp.")
            return
