...
- `Profit` has `Add Expense` for person-level expenses. Expense categories are `Travel`, `Supplies`, `Travel Meal`, and `Fees`; expenses record a date and deduct from that person's period/overall profit and charts. Expense rows are profit-only adjustments and cannot be refunded to inventory.
