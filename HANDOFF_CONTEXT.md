...
- Create has optional seller terms for buying from people at fixed rates. The shared runtime file is `ASSIGNMENT RULES\seller_terms.csv` with columns `Seller`, `Sheet Type`, `Value Source`, `Seller Rate`, and optional `Deduction`; see `seller_terms.example.csv`. If Create's Seller or Sheet Type is blank, L.U.C.A.S leaves purchase prices alone. If both are filled, it sets the Create `Purchase` column to the seller's sell price, separate from the normal best-company assignment payout. With `Seller Rate`, it uses the selected value source. With `Deduction`, it follows the `Sheet Type` company's existing assignment/payout rules per card and subtracts that deduction from the company payout, e.g. Arena Club with `5%` pays all Arena Club rule payouts minus five percentage points.
...
- `Payouts/Tabs` active balances pay seller-term people their sheet purchase total. Assigned people not listed in `seller_terms.csv` are treated as team members and their active balance is half estimated profit: `(Estimated Payout total - Purchase total) / 2`, floored at zero.
