...
- Payouts/Tabs now supports partial balance payments. Right-click a balance row and choose the payout action; the popup accepts a payment amount, records `paid_amount`, leaves any remainder open as `Partial`, and still supports paying the full open balance in one click. Team-member payouts remain per sold card/expense, while seller/network payouts can be viewed by seller sheet. Team Profit also has a `Full Profit` / `My Profit` view so Michael can see house profit after team balance-share payouts, and Home has a sheet search field under the sort control.
