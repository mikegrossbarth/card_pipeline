...
- Assignment Rules now exposes per-company `Sync Google Keep` inside each selected company's Rule Source panel. The old app-wide Keep sync flow was removed because companies can use different Keep notes.
...
- Assignment Rules has per-company `Sync Google Keep` inside each selected company's Rule Source panel, which opens that company's saved Google Keep note source so the Chrome extension can refresh the cached export. On Windows it tries Chrome directly before falling back to the default browser.
