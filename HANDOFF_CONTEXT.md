...
- Inventory filter popup now includes `Missing Comps Only` and `Missing CL Value Only`. The old large `No Comps` toolbar button was removed to keep the Inventory action row cleaner; selected missing-value filters match rows missing either selected value, still hide sold rows, and are covered by focused tests.
