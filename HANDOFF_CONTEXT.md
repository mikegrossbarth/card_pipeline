...
The app reads local `.txt`, `.md`, `.json`, `.csv`, `.xlsx`, and `.xlsm` files, including synced Google Drive paths such as `G:\My Drive\...`. The manager exposes three rule source modes: manual rules, Google Keep local file, and Google Sheets local file. Google Keep means a local synced/exported text/markdown file. Google Sheets can be a local `.xlsx`/`.csv` or a native `.gsheet` shortcut. When `.gsheet` is selected, L.U.C.A.S exports it to a cached `.xlsx` under `<pipeline root>\ASSIGNMENT RULES\SHEET EXPORTS`, stores that local workbook path, then reads the workbook. The shortcut reader supports common metadata (`url`, `doc_id`, `resource_id`). If Google Drive exposes the shortcut as an unreadable placeholder, the manager asks for the Google Sheet URL and exports from that URL instead.
