...
- Startup Home loading now uses cached workbook summaries immediately and refreshes changed/missing summaries in the background. This avoids blocking Team L.U.C.A.S startup on a cold Google Drive read of every incoming/working/received workbook.
- Team Profit person filtering now only offers explicit People Rules `Balance Share %` team members, plus `My Profit`, when balance-share rows are configured. Network sellers/non-balance-share people stay out of the Profit person selector because their payouts are seller-term sheet economics, not per-card team profit share.
- Payouts/Tabs now supports partial balance payments. Right-click a balance row and choose the payout action; the popup accepts a payment amount, records `paid_amount`, leaves any remainder open as `Partial`, and still supports paying the full open balance in one click. Team-member payouts remain per sold card/expense, while seller/network payouts can be viewed by seller sheet. In Team Profit, `My Profit` is now a special Person dropdown selection that shows Michael's house profit after team balance-share payouts, and Home has a sheet search field under the sort control.
...
- Windows was re-synced with recent shared Mac desktop comp/inventory fixes while intentionally skipping mobile/app/Instagram/CY-only work. Comp now includes Low Comp % Avg filtering for bogus low comps and Lot Price Fill for allocating lot purchase prices from Comps/Card Ladder/CY values; inventory filters now include Missing Card Description Only; clipboard-copy actions retry after stale clipboard clears. Focused tests cover these behaviors.
- Profile split groundwork: L.U.C.A.S now honors LUCAS_SETTINGS_PATH and LUCAS_ASSIGNMENT_CONFIG_PATH, so Team and Michael/private launches can use separate local settings and company configs without changing the shared app code. Windows uses hidden-window launchers Run Team LUCAS.vbs and Run Michael LUCAS.vbs so a command prompt does not stay open beside the app. Mac has Run Team LUCAS.command and Run Michael LUCAS.command. The actual profile files are intentionally ignored (lucas_settings.*.json, ssignment_companies.*.json). On this Windows machine, the ignored Michael profile points to private Drive root G:\My Drive\LUCAS_PERSONAL, with its own WORKING SHEETS, INCOMING SHEETS, RECEIVED SHEETS, ARCHIVED SHEETS, COMPANY SHEETS, ASSIGNMENT RULES, and INVENTORY PHOTOS; copied personal assignment rules currently load Arena Club, Fanatics, and Court Yard from that private folder. Team L.U.C.A.S still uses the existing shared G:\My Drive\CARD_PIPELINE.
- Personal L.U.C.A.S now removes people-assignment UI friction without changing underlying data. Profiles detected by personal/Michael/Mikey settings paths or private LUCAS_PERSONAL roots hide the Home/Inventory/Profit person controls, hide the Payouts/Tabs tab, and default new manual inventory cards and expenses to assigned person Mikey. Team L.U.C.A.S keeps person assignment, person filters, and payouts unchanged.
- Inventory photos are now linked from a phone-synced folder, with the preferred Shortcut shape being file-based: Take Photo then Save File directly into iCloud Drive/Google Drive/Shortcuts folder so L.U.C.A.S can later remove the synced file on intentional inventory deletes. L.U.C.A.S creates/uses shared CARD_PIPELINE\INVENTORY PHOTOS, or the user can click Inventory -> Photo Folder to choose an iCloud/Drive-synced phone export folder without editing JSON. Inventory -> Scan Photos now automatically copies from that selected source into shared CARD_PIPELINE\INVENTORY PHOTOS when needed, then scans the shared folder so linked photo paths are team-visible through Google Drive. New links are stored as portable paths relative to shared INVENTORY PHOTOS when possible, while old absolute links are remapped into the current shared/source folder if possible. HEIC phone exports are supported through pillow-heif, which is now in 
equirements.txt; older installs need to rerun install dependencies or export JPEGs. It also repeats that mirror-and-scan flow every three hours, skips photos already linked to active inventory rows, OCRs remaining new/changed images, links matching certs to active inventory rows, shows a Photos count, and exports photo paths. Sold cards keep their photo files available for refund/return recovery; explicit Inventory Delete archives every linked source/shared photo file it can safely resolve unless another active row still uses the same photo. The legacy Photos-album export still works, but L.U.C.A.S cannot directly delete originals from the private iPhone Photos library unless your Shortcut is built around Files/synced folders instead of keeping the original in Photos. During mirror and scan steps the Inventory status line shows per-file progress. Linked rows expose right-click Open Photo and Open Photo Folder quick actions. This intentionally uses only phone capture/synced-folder handling from the old automatic-inventory-update thread; no Instagram/R2 posting code was brought in. Setup doc: docs/INVENTORY_PHOTO_ALBUM.md.
...
- Weekly company tabs default to rolling over on Monday at 8:00 PM local time, and Company Rules can override the reset weekday/time per company. Sunday midnight no longer starts the next company-sheet week unless a company is explicitly configured that way.
...
- Card Ladder money parsing now understands `k`/`K` thousand suffixes end-to-end. Values like `$20.27k` now parse as `$20,270.00` in the Chrome extension, bridge/OCR parsers, app money parser, and spreadsheet import parser instead of leaking through as `$20.27`.
...
- Windows tabs now live inside scrollable tab canvases so shrinking the L.U.C.A.S window below the full working layout exposes vertical/horizontal scrollbars and mouse-wheel tab scrolling instead of cutting off controls. The minimum window size is reduced to `760x520`.
...
- Profit graph options now include `Profit by Company`, which groups filtered sold-card profit by company/buyer, skips expenses, and renders one trend line per company over the selected date period.
...
- Windows raw inventory receive fix: Home `All Received` sync now imports raw/no-cert rows from received sheets into inventory, preserves any sheet `Item ID`, and clears stale blank-cert inventory keys before generating missing `RAW-YYYYMMDD-####` ids. Utility scripts `scripts/backfill_raw_item_ids.py` and `scripts/sync_received_inventory_sheet.py` were added for controlled raw-ID backfills and one-sheet received inventory syncs. Live personal Windows backfill on 2026-07-08 added 91 raw IDs to Incoming/Working sheets and synced 13 rows from `RAW_INVENTORY_ADDITION_7_7_6.xlsx` into personal inventory.
...
- Weekly company-sheet tabs roll forward Monday at 8:00 PM local time by default; per-company reset weekday/time in Company Rules overrides that default.
...
- Create has local `Network Mode` in `lucas_settings.json`. When off, Create hides Person/Sheet Type and seller terms do not apply, keeping normal Open Team UI clean. When on, Create exposes optional person terms for buying from sellers at fixed rates. People Rules are edited from `Company Rules -> People Rules` and stored in the shared runtime file `ASSIGNMENT RULES\seller_terms.csv` with columns `Person`, `Sheet Type`, optional `Min Value` / `Max Value`, `Seller Rate`, `Deduction`, and `Balance Share`; legacy `Seller` headers still load. The People Rules UI labels the percentage fields as `Seller Rate %`, `Deduction %`, and `Balance Share %`; type numbers only, decimals allowed, without percent signs. Blank Min/Max bounds default to `$0` and `$1,000,000,000`. `Sheet Type` must match an assignment-rule company for Seller Rate/Deduction rows, so that company rule decides the per-card value source. A blank `Sheet Type` with `Balance Share %` defines a team member profit share and does not classify that person as a seller source. If Create's Person or Sheet Type is blank for a seller row, L.U.C.A.S leaves purchase prices alone. If both are filled, it sets the Create `Purchase` column to the seller's sell price, separate from the normal best-company assignment payout. With `Seller Rate %`, it pays that percentage of the matching company rule value. With `Deduction %`, it follows the `Sheet Type` company's existing assignment/payout rules per card and subtracts that percentage of the company source value from the company payout, e.g. Arena Club with company payout `95` on value `100` and deduction `10` writes purchase `85`.
...
- Company Rules has a `People Rules Health` panel next to `Google Status`. It checks `ASSIGNMENT RULES\seller_terms.csv` for missing/unreadable files, empty CSVs, invalid Seller Rate/Deduction/Balance Share values, overlapping Person/Sheet Type value ranges, inactive Sheet Type companies, missing Sheet Type assignment companies, and shows parsed rates/deductions/balance shares. Buttons refresh the status, open the terms folder, and copy diagnostic details.
- `Payouts/Tabs` active balances pay seller-term people their sheet purchase total only after the sheet is `Received`; seller/non-team buy payouts do not appear while the sheet is merely `Incoming`. Assigned people without seller Sheet Type terms are treated as team members, and their payout rows are generated from sold-profit entries in `profit_ledger.json`; team balance uses that person's `Balance Share %` People Rule when present and otherwise defaults to 50% of realized sold profit. Unsold estimated payout never creates team profit owed.
...
- Company Rules has per-company `Sync Google Keep` inside each selected company's Rule Source panel, which opens that company's saved Google Keep note source so the Chrome extension can refresh the cached export. On Windows it tries Chrome directly before falling back to the default browser. The local bridge intentionally keeps saved Keep URLs private in `/command` and `/status` responses; do not re-add `keepNoteSources` there, because older extensions may auto-open advertised Keep tabs.
