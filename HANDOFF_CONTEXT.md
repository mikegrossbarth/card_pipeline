...
- Inventory filter popup now includes `Missing Comps Only` and `Missing CL Value Only`. The old large `No Comps` toolbar button was removed to keep the Inventory action row cleaner; missing-value filters still hide sold rows and are covered by focused tests.
