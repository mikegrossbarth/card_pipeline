...
- Assignment has `Sync Google Keep`, which opens saved Google Keep note sources from Assignment Rules so the Chrome extension can refresh cached Keep exports without the user hunting for each note manually. On Windows it tries Chrome directly before falling back to the default browser.
