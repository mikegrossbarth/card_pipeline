...
- Profile split groundwork: L.U.C.A.S now honors `LUCAS_SETTINGS_PATH` and `LUCAS_ASSIGNMENT_CONFIG_PATH`, so Team and Michael/private launches can use separate local settings and company configs without changing the shared app code. Windows has committed launchers `Run Team LUCAS.bat` and `Run Michael LUCAS.bat`; Mac has `Run Team LUCAS.command` and `Run Michael LUCAS.command`. The actual profile files are intentionally ignored (`lucas_settings.*.json`, `assignment_companies.*.json`). On this Windows machine, the ignored Michael profile points to private Drive root `G:\My Drive\LUCAS_PERSONAL`, with its own `WORKING SHEETS`, `INCOMING SHEETS`, `RECEIVED SHEETS`, `ARCHIVED SHEETS`, `COMPANY SHEETS`, `ASSIGNMENT RULES`, and `INVENTORY PHOTOS`; copied personal assignment rules currently load Arena Club, Fanatics, and Court Yard from that private folder. Team L.U.C.A.S still uses the existing shared `G:\My Drive\CARD_PIPELINE`.
