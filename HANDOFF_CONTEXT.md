...
- Create has optional seller terms for buying from people at fixed rates. The shared runtime file is `ASSIGNMENT RULES\seller_terms.csv` with columns `Seller`, `Sheet Type`, `Value Source`, and `Seller Rate`; see `seller_terms.example.csv`. If Create's Seller or Sheet Type is blank, L.U.C.A.S leaves purchase prices alone. If both are filled, it maps the seller/sheet type to a value source/rate and sets the Create `Purchase` column to the seller's sell price, separate from the normal best-company assignment payout.
