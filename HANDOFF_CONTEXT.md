...
- All main tabs now fit at the default app size without outer tab scrollbars in runtime geometry checks. Wide/tall data stays scrollable inside the relevant table/list/chart areas instead of forcing the whole tab canvas to pan.
- Profit tab now uses a side-by-side chart and ledger layout so the graph remains usable while the tab still fits the default app window without outer scrolling.
