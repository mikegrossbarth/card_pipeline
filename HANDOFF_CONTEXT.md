...
- Inventory photos are now linked from a phone-synced folder, with the preferred Shortcut shape being file-based: `Take Photo` then `Save File` directly into iCloud Drive/Google Drive/Shortcuts folder so L.U.C.A.S can later delete the actual synced file. L.U.C.A.S creates/uses shared `CARD_PIPELINE\INVENTORY PHOTOS`, or the user can click `Inventory -> Photo Folder` to choose an iCloud/Drive-synced phone export folder without editing JSON. `Inventory -> Scan Photos` now automatically copies from that selected source into shared `CARD_PIPELINE\INVENTORY PHOTOS` when needed, then scans the shared folder so linked photo paths are team-visible through Google Drive. New links are stored as portable paths relative to shared `INVENTORY PHOTOS` when possible, while old absolute links are remapped into the current shared/source folder if possible. HEIC phone exports are supported through `pillow-heif`, which is now in `requirements.txt`; older installs need to rerun install dependencies or export JPEGs. It also repeats that mirror-and-scan flow every three hours, skips photos already linked to active inventory rows, OCRs remaining new/changed images, links matching certs to active inventory rows, shows a `Photos` count, exports photo paths, and keeps sold-card photo paths on the profit ledger so refunds can restore them to inventory. Intentional inventory deletes and move-to-company actions still delete linked source/shared photo files when no other active row uses the same photo. The legacy Photos-album export still works, but L.U.C.A.S cannot directly delete originals from the private iPhone Photos library; it deletes synced Files/Drive/iCloud copies it can see. During mirror and scan steps the Inventory status line shows per-file progress. Linked rows expose right-click `Open Photo` and `Open Photo Folder` quick actions. This intentionally uses only phone capture/synced-folder handling from the old automatic-inventory-update thread; no Instagram/R2 posting code was brought in. Setup doc: `docs/INVENTORY_PHOTO_ALBUM.md`.
- Home refresh now reconciles duplicate/accounted incoming and working sheets against inventory/profit ledgers. Fully accounted sheets are marked received, moved to `RECEIVED SHEETS`, and logged; partially accounted sheets stay put and trigger a loud warning so manually re-added incoming sheets cannot silently duplicate cards already in inventory/company/sold ledgers. Live `KEVIN_HAMLIN_DALTON.xlsx` was reconciled to Received on 2026-06-30.
