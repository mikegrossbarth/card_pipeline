...
- Inventory now has a `No Comps` toolbar button plus a `Missing Comps Only` filter popup option. Both show active inventory rows whose Comps/Card Ladder comps average is blank or unparsable while still hiding sold rows. This is available in both Windows and Mac and is covered by focused tests.
