...
const CARDLADDER_BACKGROUND_VERSION = "2026-07-21-visible-cert-result-v24";
...
async function selectGraderInPage(tabId, grader) {
  ...
  const modalState = await chrome.tabs.sendMessage(tabId, { type: "CARDLADDER_MODAL_STATE" })
    .catch(() => ({ open: false }));
  if (!modalState?.open) {
    await chrome.tabs.sendMessage(tabId, { type: "CARDLADDER_PREPARE_CERT_MODAL" })
      .catch(() => null);
  }
...
async function selectGraderWithTrustedClicks(tabId, grader) {
  ...
  try {
    ...
    for (const point of geometry.points.slice(0, 24)) {
      ...
      await debuggerMouseClick(tabId, option.clickPoint.x, option.clickPoint.y, { holdMs: 120 });
      await delay(750);
...
async function debuggerMouseClick(tabId, x, y, options = {}) {
